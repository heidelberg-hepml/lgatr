...
def test_equi_layer_norm_correctness(scale: float) -> None:
    ...
    normalized_inputs = equi_layer_norm(inputs, gain=1.0, epsilon=1e-9, config=CONFIG)
    variance = torch.mean(abs_squared_norm(normalized_inputs, config=CONFIG))
...
def test_equi_layer_norm_default_epsilon() -> None:
    ...
    variance = torch.mean(
        abs_squared_norm(equi_layer_norm(inputs, gain=1.0, config=CONFIG), config=CONFIG)
    )
    torch.testing.assert_close(variance, torch.ones_like(variance), **TOLERANCES)

    tiny = 1e-3 * inputs
    variance = torch.mean(
        abs_squared_norm(equi_layer_norm(tiny, gain=1.0, config=CONFIG), config=CONFIG)
    )
    expected = torch.mean(abs_squared_norm(tiny, config=CONFIG)) / 0.01
    torch.testing.assert_close(variance, expected, **TOLERANCES)
